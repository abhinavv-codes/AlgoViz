...
const selectText = document.querySelector('.selected') || { innerHTML: '' } // Fallback for removed element
...
resetButton.addEventListener("click", function () {
    ...
    enableSortingBtn();
    enableSizeSlider();
    selectText.innerHTML = `Array Reset`
    // Reset the array by regenerating it with the current size
    createNewArray(arrayVal);
    const description = document.querySelector('#description')
    description.style.display = 'none'
    const section = document.querySelector('#fullbody')
...
function swapping(eliment_1, eliment_2) {
    ...
    // Also swap the text content (numbers)
    let tempText = eliment_1.innerHTML;
    eliment_1.innerHTML = eliment_2.innerHTML;
    eliment_2.innerHTML = tempText;
}

// Safely updates Java code block content while preserving expected pre/code structure.
function setJavaCodeSnippet(sourceCode) {
    const javaPanel = document.querySelector('#code-java');
    if (!javaPanel) return;

    let javaCode = javaPanel.querySelector('code.language-java');
    if (!javaCode) {
        javaPanel.innerHTML = '<pre><code class="language-java"></code></pre>';
        javaCode = javaPanel.querySelector('code.language-java');
    }

    javaCode.textContent = sourceCode;
    if (window.Prism && typeof Prism.highlightElement === 'function') {
        Prism.highlightElement(javaCode);
    }
}

// Code tab switching for main code block
...
codeTabs.forEach(tab => {
    tab.addEventListener('click', function() {
        ...
        // Hide all code implementations
        const allCodeImpls = document.querySelectorAll('.code-impl');
        allCodeImpls.forEach(code => code.classList.remove('active'));
        
        // Show selected code implementation
        const selectedCodeImpl = document.getElementById('code-' + language);
        if (selectedCodeImpl) {
            selectedCodeImpl.classList.add('active');
        }
    });
});

// Dropdown menu code removed - no longer needed since dropdown was removed from HTML