...
async function descriptionText_insertion() {
    ...
    setJavaCodeSnippet(`// Java program for implementation of Insertion Sort
...
async function InsertionSort() {
    ...
    element[0].style.background = SORT_COLORS.compare;
    for (let i = 1; i < element.length; i++) {
        ...
        while (j >= 0 && (parseInt(element[j].style.height) > parseInt(p))) {
            ...
            for (let k = i; k >= 0; k--) {
                element[k].style.background = SORT_COLORS.sorted;
