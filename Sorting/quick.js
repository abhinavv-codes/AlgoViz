...
async function descriptionText_quick() {
    ...
    setJavaCodeSnippet(`// Java implementation of QuickSort
import java.io.*;
...
async function partition(element, low, high) {
    beep.play();
    let i = low - 1;
    element[high].style.background = SORT_COLORS.swap;
    for (let j = low; j <= high - 1; j++) {
        ...
        if (parseInt(element[j].style.height) < parseInt(element[high].style.height)) {
            ...
            element[i].style.background = SORT_COLORS.swap;
            if (i != j) element[j].style.background = SORT_COLORS.swap;
    ...
    element[high].style.background = SORT_COLORS.compare;
    element[i].style.background = SORT_COLORS.sorted;
    ...
    for (let k = 0; k < element.length; k++) {
        beep.play();
        if (k !== i) {
            element[k].style.background = SORT_COLORS.compare;
        }
    }
...
async function quickSort(element, low, high) {
    if (low < high) {
        ...
        if (low >= 0 && high >= 0 && low < element.length && high < element.length) {
            beep.play();
            element[high].style.background = SORT_COLORS.sorted;
            element[low].style.background = SORT_COLORS.sorted;
