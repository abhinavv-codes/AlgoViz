...
async function descriptionText_merge() {
   ...
   setJavaCodeSnippet(`/* Java program for Merge Sort */
class MergeSort {
...
async function Merge(element, low, mid, high) {
   ...
   for (let i = 0; i < a1; i++) {
      ...
      beep.play();
      element[low + i].style.background = SORT_COLORS.swap;
   ...
   for (let i = 0; i < a2; i++) {
      await waitforme(delay);
      beep.play();
      element[mid + 1 + i].style.background = SORT_COLORS.compare;
   ...
   while (i < a1 && j < a2) {
      ...
      if (parseInt(left[i]) <= parseInt(right[j])) {
         if ((a1 + a2) === element.length) {
            ...
            element[k].style.background = SORT_COLORS.swap;
